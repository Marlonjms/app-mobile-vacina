...
const App = () => {
  ...
  const fetchVacinas = async () => {
    try {
      ...
      const response = await axios.get(`http://192.168.0.129:3000/api/vacina/${cpf}`);
  ...
  const handleAddOrUpdateVacina = async () => {
    try {
      ...
      if (selectedVacinaId) {
        ...
        await axios.put(`http://192.168.0.129:3000/api/vacina/${selectedVacinaId}`, newVacina);
        Alert.alert('Sucesso!', 'Vacina atualizada com sucesso!');
      } else {
        // Adicionar nova vacina
        await axios.post('http://192.168.0.129:3000/api/vacina', newVacina);
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancelar', style: 'cancel' },
          {
            text: 'Excluir',
            onPress: async () => {
              await axios.delete(`http://192.168.0.129:3000/api/vacina/${id}`);
              fetchVacinas();
            }
          }
        ]
